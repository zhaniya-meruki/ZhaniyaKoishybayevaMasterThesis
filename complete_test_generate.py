...
class Generator(object):
	def __init__(self):
		...
		self.synthesizer = sif.Synthesizer(args.stream, verbose=False)

	def read_window(self, window_fnames, stream):
		window = []
		for fname in window_fnames:
			...
			if stream == 'both':
				thr_fname = fname.replace("rgb_roi", "thr_roi")
				thr_img = cv2.imread(thr_fname)
				thr_img_gray = cv2.cvtColor(thr_img, cv2.COLOR_BGR2GRAY)
				thr_img_gray = thr_img_gray.reshape((thr_img_gray.shape[0], thr_img_gray.shape[1], 1))
				img = np.concatenate((img, thr_img_gray), 2)
	...
	def vc(self, sample, outfile, stream):
		hp = sif.hparams
		...
		for window_idx, window_fnames in enumerate(all_windows):
			images = self.read_window(window_fnames, stream)
...
def get_image_list(split, data_root, stream):
	...
	with open(os.path.join(data_root, '{}.txt'.format(split))) as vidlist:
		for vid_id in vidlist:
			vid_id = vid_id.strip()
			filelist.extend(list(glob(os.path.join(data_root, 'preprocessed', vid_id, folder, '*.jpg'))))
	return filelist


def get_testlist(data_root, stream):
	test_images = get_image_list('test', data_root, stream)
...
if __name__ == '__main__':
	...
	parser.add_argument('-stream', "--stream", help="rgb, thr or both", required=True)
	...
	videos = get_testlist(args.data_root, args.stream)
	...
	for vid in tqdm(videos):
		...
		for (ss, es), images in tqdm(contiguous_window_generator(vidpath)):
			...
			try:
				g.vc(sample, outfile, args.stream)
			...
			command = template.format(ss, os.path.dirname(os.path.dirname(vidpath)) + '/audio.wav', es, 
