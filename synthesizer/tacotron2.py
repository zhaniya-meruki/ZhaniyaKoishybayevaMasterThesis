...
class Tacotron2:
    def __init__(self, checkpoint_path, hparams, stream, gta=False, model_name="Tacotron"):
        ...
        with tf.variable_scope("Tacotron_model") as scope:
            ...
            if gta:
                self.model.initialize(inputs, input_lengths, speaker_embeddings, stream, targets, gta=gta,
                                      split_infos=split_infos)
            else:
                self.model.initialize(inputs[0], input_lengths[0], speaker_embeddings[0], stream, split_infos=split_infos[0])
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, embed_filenames):
        ...
        feed_dict[self.speaker_embeddings] = [np.zeros([1,256], dtype=np.float32)]
