...
class Feeder:
	...
	def __init__(self, coordinator, hparams, stream):
		...
		input_size = 3    
		if stream == 'both':
			print ("Input size is set to 4 for combined model")
			input_size = 4
		...
		with tf.device("/cpu:0"):
			...
			self._placeholders = [
				tf.placeholder(tf.float32, shape=(None, hparams.T, hparams.img_size, 
													hparams.img_size, input_size), name="inputs"),
	...
	def getitem(self, split='train'):
		while 1:
			...
			mel = np.load(os.path.join('Dataset/zhanat/preprocessed/'+os.path.dirname(img_name).split(os.path.sep)[-2], 'mels.npz'))['spec'].T
