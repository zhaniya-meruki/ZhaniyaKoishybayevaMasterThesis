...
def model_train_mode(args, feeder, hparams, global_step, stream):
    with tf.variable_scope("Tacotron_model", reuse=tf.AUTO_REUSE) as scope:
        model = create_model("Tacotron", hparams)
        model.initialize(feeder.inputs, feeder.input_lengths, feeder.speaker_embeddings, stream, 
...
def model_test_mode(args, feeder, hparams, global_step, stream):
    with tf.variable_scope("Tacotron_model", reuse=tf.AUTO_REUSE) as scope:
        model = create_model("Tacotron", hparams)
        model.initialize(feeder.eval_inputs, feeder.eval_input_lengths, 
                         feeder.eval_speaker_embeddings, stream, feeder.eval_mel_targets, targets_lengths=feeder.eval_targets_lengths, 
...
def train(log_dir, args, hparams, stream):
    ...
    with tf.variable_scope("datafeeder") as scope:
        feeder = Feeder(coord, hparams, stream)
    
    # Set up model:
    global_step = tf.Variable(0, name="global_step", trainable=False)
    model, stats = model_train_mode(args, feeder, hparams, global_step, stream)
    eval_model = model_test_mode(args, feeder, hparams, global_step, stream)
...
def tacotron_train(args, log_dir, hparams, stream):
    return train(log_dir, args, hparams, stream)
